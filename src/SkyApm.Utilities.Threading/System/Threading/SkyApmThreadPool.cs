...
namespace System.Threading
{
    ...
    public static class SkyApmThreadPool
    {
        ...
        {
            return ThreadPool.QueueUserWorkItem(callBack.Diagnostic(SkyApmCategory.FORK));
        }

        /// <inheritdoc cref="ThreadPool.QueueUserWorkItem(WaitCallback, object?)"/>
        public static bool QueueUserWorkItem(WaitCallback callBack, object state)
        {
            return ThreadPool.QueueUserWorkItem(callBack.Diagnostic(SkyApmCategory.FORK), state);
        }

        /// <inheritdoc cref="ThreadPool.QueueUserWorkItem{TState}(Action{TState}, TState, bool)"/>
        public static bool QueueUserWorkItem<TState>(Action<TState> callBack, TState state, bool preferLocal)
        {
            return ThreadPool.QueueUserWorkItem(callBack.Diagnostic(SkyApmCategory.FORK), state, preferLocal);
        }
        ...
        {
            return ThreadPool.UnsafeQueueUserWorkItem(callBack.Diagnostic(SkyApmCategory.FORK), state);
        }

        /// <inheritdoc cref="ThreadPool.UnsafeQueueUserWorkItem{TState}(Action{TState}, TState, bool)"/>
        public static bool UnsafeQueueUserWorkItem<TState>(Action<TState> callBack, TState state, bool preferLocal)
        {
            return ThreadPool.UnsafeQueueUserWorkItem(callBack.Diagnostic(SkyApmCategory.FORK), state, preferLocal);
        }
    }
}
